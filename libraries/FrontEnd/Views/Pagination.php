...
final class Pagination extends AbstractView
{
    ...
    function render($query)
    {
        ...
        if ($this->theme->options['numeric_paginav']) {
    ...
    private function theNumericPagination(\WP_Query $query, $pages = [])
    {
        ...
        if ($this->theme->options['paginav_mid_size'] <= $current) {
            $pages[] = $current;
            for ($i = 1; $i <= $this->theme->options['paginav_mid_size']; $i++) {
        ...
        if ( !in_array(1, $pages) ) :
            ...
            ?><li<?= $class ?>>
                <a href="<?= get_pagenum_link(1) ?>" aria-label="<?= __('Go to page', 'banhmi') ?> 1">1</a>
            </li><?php
            if ( !in_array(2, $pages) )
                echo '<li role="presentation">&hellip;</li>';
        endif;
        ...
        foreach ($pages as $page) :
            ...
            ?><li<?= $class ?>>
                <a href="<?= get_pagenum_link($page) ?>" aria-label="<?= __('Go to page', 'banhmi') . ' ' . $page ?>"><?= $page ?></a>
            </li><?php
        endforeach;
        ...
        if ( !in_array($maxpages, $pages) ) :
            ...
            ?><li<?= $class ?>>
                <a href="<?= get_pagenum_link($maxpages) ?>" aria-label="<?= __('Go to page', 'banhmi') . ' ' . $maxpages ?>"><?= $maxpages ?></a>
            </li><?php
        endif;
    }
    ...
    {
        ?><li class="current">
            <?= __('Page', 'banhmi') . ' ' . $query->query_vars['paged'] . '/' . $query->max_num_pages ?>
        </li><?php
    }
}
