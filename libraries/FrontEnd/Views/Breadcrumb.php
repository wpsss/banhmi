...
final class Breadcrumb extends AbstractView
{
    /**
     * Render
     *
    ...
    function render($query)
    {
        ?><div class="entry-breadcrumb" xmlns:v="http://rdf.data-vocabulary.org/#" itemprop="breacrumb" itemscope itemtype="https://schema.org/BreadcrumbList">
            ...
            <?= ' ' . $this->theme->options['breadcrumb_sep'] ?>
    ...
    private function theCatCrumb(\WP_Query $query, $crumb = '')
    {
        ...
        echo $this->getTypeCrumb($query) . $crumb . $label = __('Categorized as', 'banhmi') . ': ' . Html::encode($query->queried_object->name);
    }
    ...
    {
        echo $this->getTypeCrumb($query) . __('Tagged with', 'banhmi') . ': ' . Html::encode($query->queried_object->name);
    }
    ...
    {
        echo $this->getTypeCrumb($query) . __('Classified as', 'banhmi') . ': ' . Html::encode($query->queried_object->name);
    }
    ...
    {
        echo $this->getTypeCrumb($query) . __('Posted in', 'banhmi') . ': ' . $query->query_vars['year'];
    }
    ...
    private function theMonthCrumb(\WP_Query $query)
    {
        ...
        echo $this->getCrumbLink( get_year_link($year), $year ) . $this->getSep() . __('Posted on', 'banhmi') . ': ' . $month;
    }
    ...
    {
        echo __('Posted by', 'banhmi') . ': ' . $query->queried_object->display_name;
    }
    ...
    {
        echo __('Search results for', 'banhmi') . ': ' . Html::encode($query->query_vars['s']);
    }
    ...
    {
        return '</span> ' . $this->theme->options['breadcrumb_sep'] . ' <span rel="v:child" typeof="v:Breadcrumb">';
    }
